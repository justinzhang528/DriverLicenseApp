...
function MultiChoiceRuleResultPage() {
    const [dataCounts] = useState(signImg.dataCounts);
    const [problems] = useState(JSON.parse(localStorage.getItem('choiceRuleProblems') || '{}'));
    const [chooseValues] = useState((localStorage.getItem('userChoiceRuleValues')  || '{}').split(',').map(Number));
...
export default MultiChoiceRuleResultPage;