...
import MultiChoiceRegulationResultPage from './MultiChoiceRegulationResultPage';
import { AdMob } from '@capacitor-community/admob';

function MultiChocieRegulationPage() {
      ...
      buttons: [
        {
          ...
          handler: () => {
            setSelectedValue('0');
            setProblemNum(1);
            localStorage.setItem('choiceRegulationProblems',JSON.stringify(problems));
            localStorage.setItem('userChoiceRegulationValues',chooseValues.toString());
            chooseValues.splice(0);
            const navLink = document.querySelector('#goToChoiceRegulationResultPage');
  ...
  return (
    <>
      <IonHeader>
        ...
        <IonTitle>
          <IonIcon icon={listCircle} />&nbsp;&nbsp;Choice Regulation
        </IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent className="ion-text-center">
        ...
        <IonNavLink id='goToChoiceRegulationResultPage' routerDirection="forward" component={() => <MultiChoiceRegulationResultPage />}>
        </IonNavLink>
      </IonContent>
    </>
  );
}
export default MultiChocieRegulationPage;