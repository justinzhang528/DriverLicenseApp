...
import MultiChoiceRuleResultPage from './MultiChoiceRuleResultPage';
import { AdMob } from '@capacitor-community/admob';

function MultiChocieRulePage() {
      ...
      buttons: [
        {
          ...
          handler: () => {
            setSelectedValue('0');
            setProblemNum(1);
            localStorage.setItem('choiceRuleProblems',JSON.stringify(problems));
            localStorage.setItem('userChoiceRuleValues',chooseValues.toString());
            chooseValues.splice(0);
            const navLink = document.querySelector('#goToChoiceRuleResultPage');
  ...
  return (
    <>
      <IonHeader>
        ...
        <IonTitle>
          <IonIcon icon={listCircle} />&nbsp;&nbsp;Choice Rule
        </IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent className="ion-text-center">
        ...
        <IonNavLink id='goToChoiceRuleResultPage' routerDirection="forward" component={() => <MultiChoiceRuleResultPage />}>
        </IonNavLink>
      </IonContent>
    </>
  );
}
export default MultiChocieRulePage;