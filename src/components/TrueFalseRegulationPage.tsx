...
import TrueFalseRegulationResultPage from './TrueFalseRegulationResultPage';
      ...
      buttons: [
        {
          ...
          handler: () => {
            setSelectedValue('0');
            setProblemNum(1);
            localStorage.setItem('trueFalseRegulationProblems',JSON.stringify(problems));
            localStorage.setItem('userTrueFalseRegulationValues',chooseValues.toString());
            chooseValues.splice(0);
            const navLink = document.querySelector('#goToTrueFalseRegulationResultPage');
  ...
  return (
    <>
      <IonHeader>
        ...
        <IonTitle>
          <IonIcon icon={checkmarkDoneCircle} />&nbsp;&nbsp;True/False Regulation
        </IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent className="ion-text-center">
        ...
        <IonNavLink id='goToTrueFalseRegulationResultPage' routerDirection="forward" component={() => <TrueFalseRegulationResultPage />}>
        </IonNavLink>
      </IonContent>
    </>
  );
